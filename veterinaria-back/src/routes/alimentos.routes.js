...
router.get('/:id', async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT * FROM alimentos WHERE id=?', [req.params.id]);
...
router.get('/:id/foto', async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT foto_mime, foto_blob FROM alimentos WHERE id=?', [req.params.id]);
...
router.post('/', requireAuth, isAdmin, upload.single('foto'), async (req, res) => {
    try {
        ...
        const foto_mime = req.file?.mimetype || null;
        const foto_blob = req.file?.buffer || null;
        const [r] = await pool.query(
            'INSERT INTO alimentos (nombre, descripcion, precio, stock, activo, foto_mime, foto_blob) VALUES (?,?,?,?,?,?,?)',
            [nombre, descripcion || null, Number(precio) || 0, Number(stock) || 0, Number(activo ?? 1), foto_mime, foto_blob]
        );
...
router.put('/:id', requireAuth, isAdmin, upload.single('foto'), async (req, res) => {
    try {
        ...
        const [r] = await pool.query(`UPDATE alimentos SET ${sets.join(', ')} WHERE id=?`, vals);
...
router.delete('/:id', requireAuth, isAdmin, async (req, res) => {
    try {
        const [r] = await pool.query('DELETE FROM alimentos WHERE id=?', [req.params.id]);
