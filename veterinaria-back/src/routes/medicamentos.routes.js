...
router.get('/', async (req, res) => {
    try {
        ...
        const sql = `
      SELECT id, nombre, descripcion, precio, stock, requiere_rx, activo, created_at, updated_at
      FROM medicamentos
...
router.get('/:id/foto', async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT foto_mime, foto_blob FROM medicamentos WHERE id=?', [req.params.id]);
...
router.post('/', requireAuth, isAdmin, upload.single('foto'), async (req, res) => {
    try {
        const { nombre, descripcion, precio, stock, activo, requiere_rx } = req.body;
        if (!nombre) return res.status(400).json({ error: 'nombre requerido' });
        const [r] = await pool.query(
            'INSERT INTO medicamentos (nombre, descripcion, precio, stock, activo, requiere_rx, foto_mime, foto_blob) VALUES (?,?,?,?,?,?,?,?)',
            [nombre, descripcion || null, Number(precio) || 0, Number(stock) || 0, Number(activo ?? 1), Number(requiere_rx ? 1 : 0), req.file?.mimetype || null, req.file?.buffer || null]
        );
...
router.put('/:id', requireAuth, isAdmin, upload.single('foto'), async (req, res) => {
    try {
        const { nombre, descripcion, precio, stock, activo, requiere_rx, foto_clear } = req.body;
        const sets = [], vals = [];
        ...
        if (requiere_rx !== undefined) { sets.push('requiere_rx=?'); vals.push(Number(requiere_rx) ? 1 : 0); }
        ...
        const [r] = await pool.query(`UPDATE medicamentos SET ${sets.join(', ')} WHERE id=?`, vals);
...
router.delete('/:id', requireAuth, isAdmin, async (req, res) => {
    try {
        const [r] = await pool.query('DELETE FROM medicamentos WHERE id=?', [req.params.id]);
