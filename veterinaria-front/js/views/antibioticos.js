      ...
      <tbody>
        ${rows.map(r => `
          <tr data-id="${r.id}">
            ...
            <td>
              ${r.enviado_sag_bool ? `<span class="btn-env">Enviado</span>` : `<span class="btn-noenv">No enviado</span>`}
              ${r.enviado_at ? `<div class="muted" style="font-size:.8rem">${fmtDateTime(r.enviado_at)}</div>` : ''}
            </td>
            <td>
              <div class="actions">
                ${r.enviado_sag_bool
                ? `<button class="btn-noenv" data-id="${r.id}">Desmarcar</button>`
                : `<button class="btn-env" data-id="${r.id}">Marcar enviado</button>`
            }
              </div>
            </td>
          </tr>
