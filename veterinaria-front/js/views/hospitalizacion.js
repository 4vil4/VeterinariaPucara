    ...
    <tbody>
      ${rows.map(r => `
        <tr data-id="${r.id}">
          ...
          <td>${esc(r.motivo || '')}</td>
          <td>${esc(r.estado || '')}</td>
          <td><button class="btn-hosp" data-id="${r.id}">Monitoreo</button></td>
        </tr>
      `).join('')}
    </tbody>`;
  ...
  `;
    ...
    wrap.querySelector('#f_guardar').onclick = async () => {
        const data = {
            ...
            fecha_ingreso: wrap.querySelector('#fecha_ingreso').value || null, // datetime-local -> "YYYY-MM-DDTHH:mm"
...
function openHospMonitoreoCard(root, API, hospId, mountAfterEl) {
    ...
    card.innerHTML = `
    <h3 class="hosp-title">Seguimiento diario · Hospitalización #${hospId}</h3>
  ...
  `;
    ...
    // Obtener fecha ingreso para calcular "Día N"
    (async () => {
        try {
    ...
    function renderDays() {
        ...
        const d = daysBetween(hospIngresoISO, elFecha.value || hoy);
        elDays.innerHTML = `<span class="hosp-badge">Día ${d} de hospitalización</span>`;
    }
    ...
    function getRowData(tr) {
        const obj = { hora: '', fecha: elFecha.value };
    ...
    async function delRow(tr) {
        ...
        if (!hora) return;
        await fetchJSONAuth(`${API}/api/hospitalizacion/${hospId}/monitoreo?fecha=${elFecha.value}&hora=${hora}`, { method: 'DELETE' });
        tr.remove();
    }
