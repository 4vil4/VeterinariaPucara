export async function init({ root, API, authHeaders }) {
            ...
            <tr>
              ...
              <td>
                <div class="actions">
                  ...
                  <a class="btn btn-outline btn-sm" target="_blank" href="${API}/api/certificados/salud-pucara/${r.id}/pdf">PDF</a>
                  <a class="btn btn-wa btn-sm" target="_blank" href="https://wa.me/?text=${encodeURIComponent(`Le comparto su Certificado Pucará (ID ${r.id}). PDF: ${API}/api/certificados/salud-pucara/${r.id}/pdf`)}">WhatsApp</a>
  ...
  async function openForm(id = null) {
    ...
    card.innerHTML = `
      ...
      <div class="form-grid">
        <div>
          <label>Mascota</label>
          <select class="input" id="m_sel">${mascotas.map(m => `<option value="${m.id}">${esc(m.nombre)}</option>`).join('')}</select>
        </div>
        <div>
          <label>Propietario</label>
          <select class="input" id="p_sel">${propietarios.map(p => `<option value="${p.id}">${esc(p.nombre)}</option>`).join('')}</select>
        </div>
        <div>
          <label>Veterinario</label>
          <select class="input" id="v_sel">${veterinarios.map(v => `<option value="${v.id}">${esc(v.nombre)}</option>`).join('')}</select>
        </div>
        <div>
          <label>Fecha</label>
    ...
    if (editing) {
      ...
      card.querySelector('#p_sel').value = String(editing.propietario_id);
    ...
    card.querySelector('#f_guardar').onclick = async () => {
      const payload = {
        mascota_id: Number(card.querySelector('#m_sel').value),
        propietario_id: Number(card.querySelector('#p_sel').value),
        veterinario_id: Number(card.querySelector('#v_sel').value),
  ...
  async function jdel(u) {
    const r = await fetch(u, { method: 'DELETE', headers: getAuthHeaders() });
    if (!r.ok) throw new Error('HTTP ' + r.status);
    return r.json();
  }
}
