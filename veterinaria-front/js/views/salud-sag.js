            ...
            <tr>
              ...
              <td>
                <div class="actions">
                  ...
                  <a class="btn btn-outline btn-sm btn-pdf" target="_blank" href="${API}/api/certificados/salud-sag/${r.id}/pdf">PDF</a>
  ...
  async function openForm(editId = null) {
    ...
    function renderVets() {
      vSel.innerHTML = veterinarios
        .map(v => `<option value="${v.id}">${esc(v.nombre)}${v.rut ? ' — ' + esc(v.rut) : ''}</option>`)
        .join('');
